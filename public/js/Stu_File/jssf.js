...
if (!((jsf && jsf.specversion && jsf.specversion >= 20000) && (jsf.implversion && jsf.implversion >= 3))) {
    ...
    jsf.ajax = function () {
        ...
        var getTransport = function getTransport(context) {
            var returnVal;
            if (typeof context !== "undefined" && context !== null && context.form.enctype === "multipart/form-data") {
        ...
        FrameTransport.prototype = {
            setRequestHeader: function (key, value) {
                ...
                for (var i = 0; i < dataArray.length; i++) {
                    ...
                    if (nameValue[0] === "javax.faces.source" || nameValue[0] === "javax.faces.partial.event" || nameValue[0] === "javax.faces.partial.execute" || nameValue[0] === "javax.faces.partial.render" || nameValue[0] === "javax.faces.partial.ajax" || nameValue[0] === "javax.faces.behavior.event") {
    ...
    jsf.getViewState = function (b) {
        ...
        var a = function (j, l) {
            ...
            i += encodeURIComponent(j) + "=" + encodeURIComponent(l);
            k.push(i)
        };
        for (var f = 0; f < g; f++) {
            var c = d[f];
    ...
    jsf.getClientWindow = function (d) {
        ...
        var h = function h(o) {
            var p = o["javax.faces.ClientWindow"];
            if (p) {
                return p
            } else {
                var l = o.elements;
                for (var k = 0, n = l.length; k < n; k++) {
                    var m = l[k];
                    console.log("!@#$ formElement.name=" + m.name);
                    if (m.name.indexOf("javax.faces.ClientWindow") >= 0) {
                        return m
                    }
                }
            }
        ...
        var f = function (k) {
            ...
            if (k.querySelectorAll) {
                return k.querySelectorAll(b)
            }
            for (var l = k.childNodes.length - 1; l >= 0; l--) {
        ...
        var j = (d && (typeof d == "string" || d instanceof String)) ? document.getElementById(d) : (d || null);
        var a = f(j);
        var i = g(a);
        return (null != i) ? i : c()
    };
